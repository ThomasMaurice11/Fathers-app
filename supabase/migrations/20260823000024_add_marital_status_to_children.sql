-- ============================================================
-- 000024: marital_status + marriage_date on children
-- ============================================================
...
alter table public.children
  add column if not exists marital_status public.marital_status not null default 'single';

alter table public.children
  add column if not exists marriage_date date null;

-- marriage_date only allowed when married; clear conceptually enforced by check
alter table public.children
  drop constraint if exists children_marriage_date_matches_status;

alter table public.children
  add constraint children_marriage_date_matches_status
  check (
    (marital_status = 'single' and marriage_date is null)
